...
export function RandomModeClient({ plan }: { plan: PlanType }) {
  const router = useRouter();
  const [count, setCount] = useState<Count>(20);
  const limit = getDailyLimitForPlan(plan);
  const availableCounts = COUNT_OPTIONS.filter(
    (option) => limit === null || option <= limit,
  );
      ...
      <section className="space-y-3">
        ...
        {limit !== null ? (
          <p className="text-[11px] leading-relaxed text-[var(--text-3)]">
            現在のプランでは、1日あたり最大{limit}問まで演習できます。
          </p>
        ) : null}
      </section>
